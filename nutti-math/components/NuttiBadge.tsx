...
export default function NuttiBadge({ mood='happy' }: { mood?: 'happy'|'thinking'|'excited' }) {
  ...
  return (
    <div aria-hidden className="flex items-center gap-3 bg-white/80 rounded-xl p-2 border border-nutti-beige shadow-sm">
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <span className="font-bold text-nutti-orange text-lg">{t('nutti.name')}</span>
          <span className="text-lg">{moodEmojis[mood]}</span>
        </div>
        <div className="text-xs text-nutti-teal font-medium">
          {t(`nutti.moods.${mood}`)}
        </div>
      </div>
